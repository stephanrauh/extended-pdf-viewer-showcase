...
/**
 * Prototype for issue #3230 ("move and resize annotations without entering
 * annotation editor mode").
 *
 * ## The problem
 * pdf.js has no "select only" editor mode. To grab an existing annotation you
 * must be in the paint tool, where every click on the empty page creates a new,
 * unwanted annotation — the "artifacts" the reporter has to hunt down.
 *
 * ## The trick — a user-space "drag mode", no fork changes
 * pdf.js binds its create/draw-start handlers in the *bubble* phase on the
 * `.annotationEditorLayer`, and only creates when the press lands on the empty
 * layer itself. So we add our own **capture-phase** listeners that swallow
 * exactly those empty-canvas presses; presses on an existing editor keep their
 * target and so still drag / resize.
 *
 * ## Interaction — driven by pdf.js's own event, not a native double-click
 * We do NOT listen for the DOM `dblclick`: in NONE the editor layer is inert and
 * pdf.js detects the double-click synthetically from pointer timestamps, so a
 * real `dblclick` often never fires. Instead we listen on the event bus for
 * `switchannotationeditormode`, which pdf.js dispatches whenever a mode change
 * happens. The double-click on an *existing* annotation carries an `editId`
 * (`annotation_layer.js` line 882 for saved ones, `editor.dblclick → updateToolbar`
 * for freshly-drawn); a toolbar tool pick and our own `switchAnnotationEdtorMode`
 * calls do not. So `editId != null` is exactly "the user double-clicked an
 * annotation" — and toggles us drag ⇄ paint. FreeText (mode 3) is exempt so its
 * double-click edits text. The toolbar button toggles the same way.
 */
@Component({
  selector: 'app-drag-mode',
  standalone: true,
  templateUrl: './drag-mode.component.html',
  imports: [NgxExtendedPdfViewerModule, DemoComponent, Ie11MarkdownComponent],
})
export class DragModeComponent implements OnDestroy {
  ...
  /** Which source-code tab is shown below the demo. */
  public activeTab: 'html' | 'typescript' = 'html';
